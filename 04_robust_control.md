...
先在四个区间顶点和名义点上计算开环谱横坐标、闭环谱横坐标、Lyapunov 残差最大特征值以及从 $\omega$ 到 $z$ 的频域增益估计值。结果如下表所示：
...
四个顶点的开环谱横坐标都为正，说明不确定系统在整个区间内都存在明显不稳定趋势。加上同一个反馈增益以后，四个顶点和名义点的闭环谱横坐标全部转为负值，且公共 Lyapunov 残差也保持负定。对这组参数，顶点上的最坏频域增益估计约为
...
三组曲线都保持有界并逐步收敛到零附近。按脚本计算，三种代表性情形下的状态峰值分别约为 $0.1587$、$0.1673$、$0.1744$。
